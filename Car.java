...
public class Car extends JPanel{
	...
	//--------------------------------------------------------------------
	String[] comboMarka = { "","АУДИ", "БМВ","ХОНДА","ОПЕЛ" };  //комбо с марки 
	JComboBox<String> typeMarka = new JComboBox<String>(comboMarka);
	//------------------------------------------------------------------------
	String selectModel = " "; // модел Корса,Прелюд,А7
	//--------------------------------------------------------------------------
	String[] comboAudi = { "","А3", "А4", "А7","ТТ","Q7" }; 				 //комбо с модели за Ауди
	...
	//------------------------------------------------------------------------------
	String[] comboHonda = { "","Акорд", "Сивик", "Плелюд","S2000" }; 	//комбо с модели за Хонда
	JComboBox<String> typeHonda = new JComboBox<String>(comboHonda);
	//-------------------------------------------------------------------------------
	String[] comboOpel = { "","Астра", "Корса", "Омега",};   			//комбо с модели за Опел
	...
	public Car(){
		// add to Frame
			...
			//midPanel
				...
				//addButton.addActionListener(new addToTable());
				midPanel.add(updateButton);
				//updateButton.addActionListener(new updateTable());
	}
	
	class SelectType implements ActionListener { //"КУПЕ", "СЕДАН", "КОМБИ","ХЕЧБЕК","КАБРИО","ДЖИП"
	...
	class SelectMarka implements ActionListener { // sets Selected type to car type before adding in table 1
	...
	class SelectModel implements ActionListener { // sets Selected type to car type before adding in table 1

		@Override
		public void actionPerformed(ActionEvent e) {
			String selectedItem = (String) typeOpel.getSelectedItem();
